...
class RitoTestFramework():
    ...
    Individual rito test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: ritods were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a ritod test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple ritod test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'ritod exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "ritod should have exited with an error"
                else:
                    assert_msg = "ritod should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("RitoRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run ritods:
            for i in range(MAX_NODES):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in rito.conf
...
class ComparisonTestFramework(RitoTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("RITOD", "ritod"),
                          help="ritod binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("RITOD", "ritod"),
                          help="ritod binary to use for reference nodes (if any)")
