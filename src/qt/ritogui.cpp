...
const std::string RitoGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString RitoGUI::DEFAULT_WALLET = "~Default";

RitoGUI::RitoGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void RitoGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Rito address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and rito: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to RITO addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Rito addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Rito addresses"));
    ...
    openAction->setStatusTip(tr("Open a rito: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Rito command-line options").arg(tr(PACKAGE_NAME)));
...
void RitoGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/ritocointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* ritoLabelLayout = new QVBoxLayout(toolbarWidget);
        ritoLabelLayout->addWidget(label);
        ritoLabelLayout->addWidget(toolbar);
        ritoLabelLayout->setDirection(QBoxLayout::TopToBottom);
        ritoLabelLayout->addStretch(1);
...
void RitoGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool RitoGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool RitoGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void RitoGUI::setWalletActionsEnabled(bool enabled)
{
...
void RitoGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void RitoGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void RitoGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void RitoGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void RitoGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void RitoGUI::openClicked()
{
...
void RitoGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void RitoGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void RitoGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RitoGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RitoGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RitoGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RITO START */
void RitoGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void RitoGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void RitoGUI::gotoManageAssetsPage()
{
...
void RitoGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Rito network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void RitoGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void RitoGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void RitoGUI::updateHeadersSyncProgressLabel()
{
...
void RitoGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void RitoGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Rito"); // default title
...
void RitoGUI::changeEvent(QEvent *e)
{
...
void RitoGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RitoGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RitoGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "RITO")
        msg += tr("Amount: %1\n").arg(RitoUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(RitoUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void RitoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RitoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RitoGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RitoGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RitoGUI::setHDStatus(int hdEnabled)
{
...
void RitoGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void RitoGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RitoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RitoGUI::detectShutdown()
{
...
void RitoGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void RitoGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void RitoGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RitoGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void RitoGUI::subscribeToCoreSignals()
{
...
void RitoGUI::unsubscribeFromCoreSignals()
{
...
void RitoGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<RitoUnits::Unit> units = RitoUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const RitoUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(RitoUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(RitoUnits::name(u)), this);
...
{
    setText(RitoUnits::name(newUnits));
}
...
void RitoGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=RITOBTC"));
