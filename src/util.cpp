...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Rito
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Rito
    // Mac: ~/Library/Application Support/Rito
    // Unix: ~/.rito
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Rito";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Rito";
#else
    // Unix
    return pathRet / ".rito";
#endif
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No rito.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override rito.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", RITO_PID_FILENAME));
