...
The RPM spec file provided here is for Rito-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://rito.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Rito git repository](https://github.com/RitoProject/Ritocoin) but are not part of
...
Currently only `Source10` is used, for the example `rito.conf` file.
...
in the rito git repository. At present only an SVG version of the Rito
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the rito
...
needed when 0.12.1 is released, a proper fix is already in the Rito git
master branch.
...
idea to build Rito Core against a static version of that library compiled
...
The `rito.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `rito-server` package. At this point, most RPM based
...
The `rito-server` package should have SELinux support. How to properly do
...
Rito-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/rito/). He has been
packaging Rito for Fedora at least since 2012.
...
4. I renamed the `rito` package that contains the Qt GUI to `rito-core` as
that appears to be how the general population refers to it, in contrast to
`rito-xt` or `rito-classic`. I wanted to make sure the general population
